  ...
  try {
    ...
    for (const s of slides) {
      ...
      switch (s.type) {
        ...
        case "section": {
          sl.addText((s.title || "").toUpperCase(), { x: 0.8, y: 0.9, w: 8.4, h: 1.6, fontSize: 48, color: C.w, fontFace: "Arial", bold: true, align: "center", charSpacing: 4 });
          if (s.subtitle) {
            sl.addText(s.subtitle, { x: 1, y: 2.65, w: 8, h: 0.9, fontSize: 18, color: C.v3, fontFace: "Georgia", italic: true, align: "center", lineSpacingMultiple: 1.3 });
          }
          if (s.phases) {
            const phColors = [C.v1, C.v2, C.mg];
            s.phases.forEach((ph, i) => {
              pill(sl, 1.5 + i * 2.5, 3.8, 2.2, 0.48, ph, { bg: phColors[i], color: C.w, fontSize: 12, shadow: false });
            });
          }
          addLogo(pres, sl);
          break;
        ...
        case "positionnement": {
          ...
          const hasOdd = elems.length % 2 !== 0;
          const cardH = elems.length <= 4 ? 1.2 : 0.92;
          const rowGap = elems.length <= 4 ? 1.38 : 1.05;
          const startY = elems.length <= 4 ? 2.0 : 1.85;
          elems.forEach((el, i) => {
            const isLastOdd = hasOdd && i === elems.length - 1;
            const col = i % 2;
            const row = Math.floor(i / 2);
            const xP = isLastOdd ? 2.5 : (0.8 + col * 4.3);
            const wP = isLastOdd ? 5.0 : 4.0;
            const yP = startY + row * rowGap;
            cardShape(pres, sl, xP, yP, wP, cardH, { accentColor: [C.v1, C.v2, C.mg, C.pk, C.v1][i] || C.v1 });
            numCircle(pres, sl, xP + 0.15, yP + (cardH/2 - 0.2), 0.4, i + 1, { fontSize: 12 });
            sl.addText(el, { x: xP + 0.7, y: yP + 0.1, w: wP - 0.8, h: cardH - 0.15, fontSize: 11, color: C.g1, fontFace: "Arial", valign: "middle", lineSpacingMultiple: 1.3 });
          });
          addLogo(pres, sl);
          break;
        }
        ...
        case "calendrier": {
          ...
          // Nouvelle structure : s.semaines[{plateforme, s1, s2, s3, s4}] OU s.plateformes (legacy)
          const calData = s.semaines || null;
          const platList = calData
            ? calData.map(r => r.plateforme)
            : (s.plateformes || ["LinkedIn", "YouTube Shorts", "TikTok"]);
          const platColors = [C.v1, C.v2, C.mg];
          platList.forEach((plat, i) => {
            const yRow = 1.85 + i * 1.0;
            // Label plateforme coloré
            sl.addShape(pres.shapes.RECTANGLE, { x: 0.35, y: yRow + 0.08, w: 1.05, h: 0.75, fill: { color: platColors[i % 3] } });
            sl.addText(plat, { x: 0.35, y: yRow + 0.08, w: 1.05, h: 0.75, fontSize: 8, color: C.w, fontFace: "Arial", bold: true, align: "center", valign: "middle", margin: 2 });
            // Cellules par semaine
            [0, 1, 2, 3].forEach((week) => {
              const xCell = 1.5 + week * 2.05;
              sl.addShape(pres.shapes.RECTANGLE, { x: xCell, y: yRow + 0.05, w: 1.9, h: 0.82, fill: { color: C.bg3, transparency: 20 }, line: { color: platColors[i % 3], width: 0.3, transparency: 60 } });
              // Contenu textuel si disponible
              let cellTxt = "";
              if (calData && calData[i]) {
                cellTxt = calData[i]["s" + (week + 1)] || "";
              }
              if (cellTxt) {
                sl.addText(cellTxt, { x: xCell + 0.05, y: yRow + 0.08, w: 1.8, h: 0.72, fontSize: 8, color: C.g1, fontFace: "Arial", valign: "middle", lineSpacingMultiple: 1.2, margin: 2 });
              }
            });
          });
          addLogo(pres, sl);
        ...
        case "tournage": {
          sl.addText("DÉROULEMENT DU TOURNAGE", { x: 0.8, y: 0.35, w: 8.4, h: 0.5, fontSize: 24, color: C.w, fontFace: "Arial", bold: true, charSpacing: 2, align: "center" });
          // Si des étapes structurées sont fournies
          if (s.etapes && s.etapes.length) {
            const tColors = [C.v1, C.v2, C.mg, C.pk];
            s.etapes.forEach((e, i) => {
              const col = i % 2, row = Math.floor(i / 2);
              const xP = 0.5 + col * 4.8, yP = 1.1 + row * 1.85;
              cardShape(pres, sl, xP, yP, 4.4, 1.6, { accentColor: tColors[i % 4] });
              numCircle(pres, sl, xP + 0.18, yP + 0.57, 0.45, i + 1, { bg: tColors[i % 4], fontSize: 14 });
              sl.addText(e.titre || "", { x: xP + 0.78, y: yP + 0.12, w: 3.45, h: 0.38, fontSize: 12, color: C.v3, fontFace: "Arial", bold: true, margin: 0 });
              sl.addText(e.description || "", { x: xP + 0.78, y: yP + 0.52, w: 3.45, h: 0.95, fontSize: 10, color: C.g1, fontFace: "Arial", valign: "top", lineSpacingMultiple: 1.35, margin: 0 });
            });
          } else {
            // Fallback : grand bloc texte
            cardShape(pres, sl, 0.8, 1.1, 8.4, 3.9);
            sl.addText(s.description || "", { x: 1.05, y: 1.25, w: 8.0, h: 3.6, fontSize: 13, color: C.g1, fontFace: "Arial", valign: "middle", lineSpacingMultiple: 1.7, align: "center" });
          }
          addLogo(pres, sl);
          break;
        }
